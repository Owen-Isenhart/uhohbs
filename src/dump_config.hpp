...
public:
	...
	void set_delay_seconds(std::uint16_t seconds) { delaySeconds = std::clamp<std::uint16_t>(seconds, 1, 300); }
	...
	void load_settings(obs_data_t *data)
	{
		...
		if (obs_data_has_user_value(data, kModeKey)) {
			...
			set_mode(modeRaw == static_cast<long long>(dump_mode::Mode::Fill) ? dump_mode::Mode::Fill
											  : dump_mode::Mode::Cut);
		}
