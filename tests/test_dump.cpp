...
template <typename T>
std::string to_string_fallback(const T &value)
{
...
void expect_equal(const Actual &actual, const Expected &expected, const char *actualExpr, const char *expectedExpr, const char *file,
    int line)
{
    if (!(actual == expected)) {
        std::ostringstream stream;
        stream << file << ":" << line << " expected " << actualExpr << " == " << expectedExpr << " (actual="
               << to_string_fallback(actual) << ", expected=" << to_string_fallback(expected) << ")";
